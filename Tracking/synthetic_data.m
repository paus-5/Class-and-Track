...
d_interp = @(t) 0.2;
s_in_interp = @(t) 0.5;
yields_AOB = 1/0.251*ones(nA,1);
yields_NOB = 1/0.062*ones(nB,1);
...
dynamic = @(t,X) [diag(growth1(X) - d_interp(t))*X(1:nA);
   ...
   (s_in_interp(t) - X(n+1))*d_interp(t) - yields_AOB'*diag(growth1(X))*X(1:nA);
    -X(n+2)*d_interp(t) + yields_AOB'*diag(growth1(X))*X(1:nA)-yields_NOB'*diag(growth2(X))*X(nA+1:n);
    -X(n+3)*d_interp(t) + yields_NOB'*diag(growth2(X))*X(nA+1:n);];
