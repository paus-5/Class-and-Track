...
Kinetic parameters of case study 2 can be seen in table \ref{kinetic_parameters}. Interaction matrices of each case are shown in figure \ref{Interactions}. Case 1 was taken from Dumont \textit{et al.}\cite{Dumont2016} where they proposed a distribution of parameters obtained with a Bayesian estimation method. Their fit describes well the dynamics of the two most abundant OTU in each functional group, but it still fails to capture the measured substrates dynamics. 
...
\begin{subfigure}[b]{0.45\textwidth}
	...
	\caption{Modified interaction matrix.}
	\label{Interaction_2}
\end{subfigure}
	\caption{Interaction matrices for each case for a consortia of 4 bacterial species where $x_1$ and $x_2$ are AOB and $x_3$ and $x_4$ are NOB. Only the entries $a_{11}$, $a_{12}$, and $a_{21}$ were modified, accounting for reciprocal positive interactions between $x_1$ and $x_2$.}
	\label{Interactions}
\end{figure} 

The ecological diagrams for each case are presented in figure \ref{ecological_diagrams}. One can see that coexistence in the same functional group is never attained for case 1, whereas for case 2 a more complex situation takes places; even a zone where more than one stable equilibria is found, implying that the outcome in that zone depends on initial conditions of the system. Note that CN can be achieved at a more elevated dilution rate in case 2 than in case 1, which can be explained by noting that the interactions of $x_1$ on $x_3$ and $x_4$ are positive and stronger in case 2, thus helping them survive through bigger dilution rates.

 Ecological diagrams also show that the model is able to overcome the competitive exclusion principle \cite{lobry2017chemostat} (CEP) paradigm. The CEP roughly states that if two species are growing on the same limiting resource, and their growth laws are non-decreasing in the resource, then only one of them will survive in the long run. However reports on wastewater treatment plants have shown coexistence between species in nitrifying reactors \cite{Wagner2002}, thus implying that a more complex growth law (as shown in here) or model structure involving other biological processes is required to include microbial diversity. 
