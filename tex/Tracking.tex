...
Finally for the parametric identification of a particular class of interaction functions an static optimization problem is proposed \eqref{Parameter Identification}. Suppose $u(t)$ is the optimal control of problem \eqref{Optimal Control}, and $x(t)$ is the associated optimal trajectory. Suppose a set of parameters $\theta \in \Theta$, such that the interaction function has a explicit form that depends on this parameters $I_{\theta}(x)$ ($\theta$ would represent matrix $A$ from section 2 for example). 
\begin{align}
