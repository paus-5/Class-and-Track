...
\begin{abstract}
	This article deals with the inclusion of microbial ecology in bioprocess modelling. The first part exposes the mathematical analysis of a model that included pairwise linear interactions fitted to experimental data in a chemostat setting. The limitations and the insights of such an approach are discussed. The main novelty of this work lies in the second part, where the use of an optimal tracking technique for the integration of genetic sequencing in chemostat models is presented. The method revisits the data used for the first model presented and proposes a way to calibrate and discriminate . 
\end{abstract}
...
The main contribution of this article is a procedure to integrate the microbial community measurements in chemostat models, based on any technique that can quantify the species abundances through time. This is a first step towards developing predictive models of a reactor's performance based on the microbial community structure.
...
Section 4 is the article's most original contribution. The concept of interaction function is developed such that it generalizes the gLV model. For the identification of the interaction function a method of optimal tracking was adapted: The growth rate of each species is modulated by a constrained regular control of the system. The regular control is composed of a feedback part on the species state variable, and a feed forward part, or tracking, on the measurement of abundances of each species. 

In section 5 the methodology from section 4 is applied to the data from the experiments perform by Dumont et al \cite{Dumont2009}. Finally, a discussion on the scope of the method, its shortcomings and perspectives is presented.
...
The model is also  able to overcome the  competitive exclusion principle \cite{lobry2017chemostat} (CEP) paradigm. The CEP roughly states that if two species are growing on the same limiting resource, and their growth law is monotonic in the substrate, then only one of them will survive in the long run. However reports on wastewater treatment plants have shown coexistence between species in nitrifying reactors has\cite{Wagner2002}.
...
The model here presented serves to illustrate that by considering a more complex growth rate that accounts for ecological interactions one might explain differences in reactors operating under similar conditions. It also shows a new mechanism by which the CEP no longer holds. However the quadratic increase of parameters with the number of OTU considered makes it difficult to accurately calibrate the model. The operating diagrams always show a single stable equilibrium point for each pair $(s_{in},D)$, however no proof is offered to such conjecture. In the next section, a generalization of the former model, and a schema for calibrating a class of models is presented as well.
...
The feedback control $v(t)$, will be of the form $v(t) = -R^{-1}\tilde{B}^\top(t)\left(\tilde{P}(t)x(t)-s_f(t)\right)$. Where matrix $P(t)$ and vector $s_f(t)$ solve differential equations. In the appendix one may prove that because of the structure of the system one only needs to calculate $2n$ differential equations for the synthesis of the control, instead of $(n+3)^2 + (n+3)$ that would imply the direct application of the method. Which renders the method- at least theoretically- scalable for a growing number of OTU.

Finally for the parametric identification of a particular class of interaction functions the following static optimization problem is proposed. Suppose $u(t)$ is the optimal control of problem \eqref{Optimal Control}, and $x(t)$ is the associated trajectory. Suppose a set of parameters $\theta \in \Theta$, such that the interaction function has a explicit form that depends on this parameters $I_{\theta}(x)$ ($\theta$ would represent matrix $A$ from section 2 for example). Then one solves the following optimization problem:
\begin{align}
\label{Parameter Identification} \min & \quad \Vert I_{\theta}(x(\cdot)) - u(\cdot) \Vert_{L^2([0,T])} \\
s.t. &  \quad \theta \in \Theta \notag \\
& \quad t \in [0,T] \notag
\end{align} 

In practice $t \in [0,T]$ from \eqref{Parameter Identification} is replaced by considering the time steps given by the differential equation solver.
\subsection{Proof of concept}

For compensating the lack of a theoretical optimal control, the approach presented in here was first tested against synthetic data. The test consisted in simulating system \eqref{Controlsystem} with $I(x) = 1 + Ax$, for $n_1 = 10$ and $n_2 = 5$. In all figures, dotted lines represent the synthetic data, while the continuous lines represent the method's output.
...
\end{bmatrix}$, found by trial and error in order for the control to fit accurately the species abundance. The rational behind choosing a smaller value for the group $G_2$ is related to the one order of magnitude difference in yields between $G_1$ and $G_2$.
...
The resulting controls correcting the growth rate can be seen in figure \eqref{Control AOB no noise} and \eqref{Control NOB no noise}. Note that from the structure of a quadratic regulator, since there is no cost in the final state, the end value is always 1. Problem \eqref{Parameter Identification} is then solved from the obtained trajectories (figures \eqref{AOB no noise} and \eqref{NOB no noise} and controls. The difference between the control and the fit is expressed in Figure \ref{Difference fit}. Not  Considering that the control is contained in $[0,1]$, the error is below 5 \% throughout the trajectory.
...
\begin{figure}
	...
	\caption{AOB trajectories using the identified matrix seen in \eqref{Comparison Interaction Matrices}.}
\end{figure}
...
\begin{figure}
	...
	\caption{NOB trajectories using the identified matrix seen in \eqref{Comparison Interaction Matrices}.}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.5 \textwidth]{Synthetic_data_no_noise//221119_no_noise_iter_5_metabolites_Iter_5_LV}
	\caption{Metabolites trajectories using the identified matrix seen in \eqref{Comparison Interaction Matrices}.}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[width= 0.4\textwidth]{260919_Try1_metabolites_Iter_110}
...
\begin{figure}[h]
	\centering
	\includegraphics[width= 0.4\textwidth]{260919_Try1_AOB_Iter_110}
...
\begin{figure}[h]
	\centering
	\includegraphics[width= 0.4\textwidth]{260919_Try1_NOB_Iter_110}
	\caption{Abundance of $G_2$ (NOB) simulation applying the control strategy.}
	\label{NOB Simulation}
\end{figure}

One can observe that in figure \ref{ControlSimulation} the predicted $s_3$ around day 240 is much higher than the measurement. If one looks at figure \ref{NOB Simulation} it suggests attributing this extra production of $s_3$ around day 240 to the growth of OTU 24. What would have happened if OTU 24 had been classified in the other functional group? One would have seen a decrease in $s_1$, an increase in $s_2$, and a decrease in $s_3$ which could have better fitted the measurements. Thus suggesting that OTU 24 was misclassified from the method presented in \cite{Ugalde-Salas2019}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width= 0.4\textwidth]{260919_Try1_Control_AOB_Iter_110}
	\caption{Control obtained for OTU in $G_1$ (AOB).}
	\label{AOB_control}
	\includegraphics[width= 0.4\textwidth]{260919_Try1_Control_NOB_Iter_110}
	\caption{Control obtained for OTU in $G_2$ (NOB).}
	\label{NOB_control}
\end{figure}

Recalling that $u(t)$ is supposed to represent $I(x(t))$ (the interactions terms), one could try to fit different forms of $I(\cdot)$ for example $I(x) = 1 + Ax$, where  $A$ is a real valued square matrix. In figure \ref{LV_fit} the simulation of the system after the identification of matrix A is presented, suggesting that an affine interaction function is not able to capture the behaviour of the system.

\begin{figure}[h]
	\centering
	\includegraphics[width= 0.4\textwidth]{260919_Try1_metabolites_Iter_110_LV}
...
The interaction function introduced tries to address the issue of prediction from the genetic sequencing, leaving the conclusion that linear pairwise interaction model do not capture the complexity of the particular nitrifying chemostat. The question remains open to other classes of interaction functions that could prove capable of prediction.

In the experiments used, the environmental factors were controlled, thus the term is interpreted as microbial interaction. However in other contexts the correction term might also be interpreted as a non accounted phenomena ranging from virus to environmental factors (e.g. temperature variations) and not only from microbial interactions. 

   

\section{Acknowledgements}
 Alain Rapaport, CMM, Thermomic
