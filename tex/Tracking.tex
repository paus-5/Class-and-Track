...
The choice of matrices $Q$ and $R$ are always of discussion in a quadratic regulator. $Q$ represents the covariance matrix of measurements, while the interpretation of $R$ is not clear. In the simulations here presented $Q = I_n$ and $R = \lambda I_n$. $\lambda$ is varied from $10^{-1}$ to $10^{-7}$ in the supplementary material section. The case $\lambda = 0.4$ is shown in here for discussion. 
...
The resulting controls that correct the growth rate can be seen in figures \eqref{Control AOB no noise} and \eqref{Control NOB no noise}. Note that from the structure of a quadratic regulator, since there is no cost in the final state, the end value is always 1. 
