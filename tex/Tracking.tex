...
The main contribution of this article is a procedure to integrate the microbial community measurements in chemostat models, based on any sequencing technique that can quantify the species abundances through time. This is a first step towards developing predictive models of a reactor's performance based on the microbial community structure.
...
Finally for the parametric identification of a particular class of interaction functions a static optimization problem is proposed \eqref{Parameter Identification}. Suppose $u(t)$ is the optimal control of problem \eqref{Optimal Control}, and $x(t)$ is the associated optimal trajectory. Suppose a set of parameters $\theta \in \Theta$, such that the interaction function has a explicit form that depends on this parameters $I_{\theta}(x)$ ($\theta$ would represent matrix $A$ from section 2 for example). In practice $t \in [0,T]$ from \eqref{Parameter Identification} is replaced by considering the time steps given by the differential equation solver. 
\begin{align}
...
One can observe that in figure \ref{ControlSimulation} the predicted $s_3$ around day 240 is much higher than the measurement. 
